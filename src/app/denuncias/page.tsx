...
export default function DenunciasPage() {
  ...
  const successContent = trackingCode ? (
    <div className={isLoggedIn ? "max-w-md mx-auto" : "w-full max-w-md"}>
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <a href="/denuncias/consultar" className="block text-center text-sm text-indigo-600 hover:underline">
  ...
  const pageContent = trackingCode ? successContent : formContent;

  if (isLoggedIn) {
    return <AppLayout title="Canal de denuncias">{pageContent}</AppLayout>;
  }

  return (
    <div className={`min-h-screen bg-slate-50 px-4 py-10 ${trackingCode ? "flex items-center justify-center" : ""}`}>
      {pageContent}
    </div>
  );
}
